...
description: "Start an AI sourcing job from the Staplehire CLI, get job status, and fetch prospects for a role."
sidebarTitle: "Source candidates"
---

Start a sourcing job for a role, check the async job status, then list matched prospects as JSON.
...
If `created` is `false`, a job is already in flight. Check the returned `job.id` instead of starting another job.
...
| Exit code `9` | Job status wait timed out | Increase `--timeout` or check the job later |

## FAQ

### Does sourcing return results immediately?

No. Sourcing is async. Check job status before reading prospects.
...
Related: [Create a role with the Staplehire CLI](/create-role-cli) · [Get job status with the Staplehire CLI](/poll-agent-jobs) · [CLI errors](/cli-errors)