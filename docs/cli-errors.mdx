...
description: "Fix Staplehire CLI authentication, validation, permission, conflict, upstream, usage, and job status timeout errors."
...
| `9` | Job status timeout | Re-run `jobs poll` with a longer timeout |
...
Related: [Authenticate the Staplehire CLI](/authentication) · [Get job status with the Staplehire CLI](/poll-agent-jobs) · [Command reference](/commands)